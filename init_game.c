...
int init_game(t_game *game, char *map_file)
{
    ...
    if (!game->mlx)
        return (0);
    game->win = mlx_new_window(game->mlx, DISPLAY_WIDTH, DISPLAY_HEIGHT, "Raycaster");
    if (!game->win)
        return (0);
    game->screen.ptr = mlx_new_image(game->mlx, DISPLAY_WIDTH, DISPLAY_HEIGHT);
    if (!game->screen.ptr)
        return (0);
    game->screen.addr = mlx_get_data_addr(game->screen.ptr, &game->screen.bits_per_pixel,
    ...
    game->weapons[0].ptr = mlx_xpm_file_to_image(game->mlx, "./arm_1.xpm", &width, &height);
