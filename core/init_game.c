...
int	init_game(t_game *game, char *map_file)
{
	...
	game->map.door_open_texture.ptr = mlx_xpm_file_to_image(game->mlx, "./texture/door_open.xpm",
			&width, &height);
	if (!game->map.door_open_texture.ptr)
	{
		printf("problem avec door_open.xpm");
		return (0);
	}
	game->map.door_open_texture.width = width;
	game->map.door_open_texture.height = height;
	game->map.door_open_texture.addr = mlx_get_data_addr(game->map.door_open_texture.ptr,
			&game->map.door_open_texture.bits_per_pixel,
			&game->map.door_open_texture.line_length,
			&game->map.door_open_texture.endian);

	game->portal_1.is_active = 0;
	game->portal_2.is_active = 0;
	game->portal_count = 0;
	game->map.game_ptr = (void *)game;
