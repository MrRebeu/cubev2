...
int is_not_wall(t_map *map, double x, double y)
{
    ...
    if (map->matrix[map_y][map_x] == '1' || 
        ...
        map->matrix[map_y][map_x] == 'i' ||
        map->matrix[map_y][map_x] == 'd')
        return 0;
    
    // Pour les portes, vérifier si elles sont ouvertes
    if (map->matrix[map_y][map_x] == 'D')
    {
        // Si t_game est accessible, vérifier l'état de la porte
        // Sinon, considérer toutes les portes comme fermées
        t_game *game = (t_game *)map->game_ptr; // Vous devrez ajouter cette référence
        if (game)
        {
            t_door *door = find_door(game, map_x, map_y);
            if (door && door->is_open)
                return 1; // Porte ouverte - on peut passer
        }
        return 0; // Porte fermée - on ne peut pas passer
    }
...
double ray_casting(t_game *game, double radiant_angle, int column_x)
{
    ...
    while (iter < max_iterations)
    {
        ...
        // Si on touche une porte, vérifier si elle est ouverte
        if (hit_type_v == 'D')
        {
            int map_x = (int)(v.x / TILE_SIZE);
            int map_y = (int)(v.y / TILE_SIZE);
            t_door *door = find_door(game, map_x, map_y);
            
            if (door && door->is_open)
            {
                // Si la porte est ouverte, continuer le rayon
                v.x += v.step_x;
                v.y += v.step_y;
                iter++;
                continue;
        ...
        // Si on touche un mur, s'arrêter
        if (hit_type_v == '1' || hit_type_v == 'D' || hit_type_v == 'P' || 
            hit_type_v == 'i' || hit_type_v == 'd')
            break;
        
        // Sinon, continuer le rayon
    ...
    // Intersection horizontale (même logique)
    iter = 0;
    while (iter < max_iterations)
    {
        ...
        if (hit_type_h == 'D')
        {
            int map_x = (int)(h.x / TILE_SIZE);
            int map_y = (int)(h.y / TILE_SIZE);
            t_door *door = find_door(game, map_x, map_y);
        ...
        // Si on touche un mur, s'arrêter
        if (hit_type_h == '1' || hit_type_h == 'D' || hit_type_h == 'P' || 
            hit_type_h == 'i' || hit_type_h == 'd')
            break;
    ...
    // Récupération finale du type de surface touchée
    hit_type_v = get_hit_type(&game->map, v.x, v.y);
    hit_type_h = get_hit_type(&game->map, h.x, h.y);
