...
void	draw_weapon_row(t_game *game, t_render *renderer, t_img *weapon)
{
	...
	while (renderer->tex_x < weapon->width)
	{
		if (renderer->x + renderer->tex_x >= 0 && renderer
			...
			draw_weapon_pixel(game, weapon, renderer,
                renderer->tex_x, renderer->tex_y);
		renderer->tex_x++;
	}
}
        ...
        t_render *renderer, int tex_x, int tex_y)
{
	...
	renderer->tex_addr = weapon->addr + (tex_y * weapon
            ->line_length + tex_x * (weapon->bits_per_pixel / 8));
	...
	if (!(renderer->red < 10 && renderer->green < 10
        && renderer->blue < 10))
	{
		dst = game->screen.addr + ((renderer->y + tex_y)
            * game->screen.line_length + (renderer->x
                + tex_x) * (game->screen.bits_per_pixel / 8));
