...
void render_door_frame(t_game *game, int column_x, t_render *renderer, t_ray *ray)
{
    ...
    // Déterminer si ce point d'impact est sur l'encadrement de la porte
    // On peut le faire en vérifiant si le point est proche du bord de la tuile
    double rel_x = (ray->wall_hit_x - (int)(ray->wall_hit_x / TILE_SIZE) * TILE_SIZE) / TILE_SIZE;
    double rel_y = (ray->wall_hit_y - (int)(ray->wall_hit_y / TILE_SIZE) * TILE_SIZE) / TILE_SIZE;
    
    // Si le point est proche d'un bord (en haut, en bas, à gauche ou à droite)
    if (rel_x < 0.1 || rel_x > 0.9 || rel_y < 0.1 || rel_y > 0.9)
        is_frame = 1;
    
    // Calculer tex_x en fonction du point d'impact
    if (ray->hit_vertical)
        renderer->tex_x = (int)(ray->wall_hit_y) % TILE_SIZE;
    else
        renderer->tex_x = (int)(ray->wall_hit_x) % TILE_SIZE;
    ...
    while (renderer->y <= renderer->draw_end)
    {
        if (renderer->y >= 0 && renderer->y < DISPLAY_HEIGHT)
        {
            ...
            // Si c'est l'encadrement, dessiner le mur
            if (is_frame)
            {
            ...
            // Sinon, ne rien dessiner (transparent)
        }
        renderer->y++;
    }
}
